...
Au cours des deux premières parties de ce manuscrit, nous avions un cadre géométrique où les données étaient principalement des nuages de points plongés dans l'espace euclidien $\mathbb R^p$ de dimension $p$. 
...
Ce changement de nature des données justifie l'emploi de méthodes spécifiques, issues de la physique statistique et du traitement du signal, qui feront l'objet des deux chapitres suivants \ref{chap:bayes_clusters_graphes} \& \ref{chap:frangi_fissures}.
...
Une des premières tâches d'analyse que l'on peut demander est de regrouper des nœuds $V$ présentant des comportements communs ; ces clusters sont appelés \textit{communautés} en théorie des graphes. Ainsi est apparu le terme de détection de communautés sur graphe \cite{Fortunato2010, CherifiPallaSzymanskiLu2019CommunityStructure}.
Dit autrement, on cherche à faire du clustering sur l'ensemble des sommets $V$.
L'on supposera tout au long de ce chapitre et du suivant que le nombre de communautés $K \ge 2$ est donné à l'avance.



% Nous nous intéressons à l'une d'elles, la dynamique de \nom{Swendsen-Wang}\cite{SwendsenWang}.
% Étant donné un graphe et une configuration de spins, c'est-à-dire un estimateur des communautés, cette dynamique forme aléatoirement des clusters et produit une nouvelle configuration. Ce faisant, elle construit une chaîne de \nom{Markov} sur l'ensemble des configurations dont la distribution stationnaire est celle de \nom{Gibbs-Boltzmann}.

\subsection{De l'optimisation à l'échantillonnage de \nom{Gibbs}}
...
La loi cible, notée $\mu$, prend souvent la forme d'une mesure de \nom{(Boltzmann--)Gibbs} \cite{Parisi} :
\[
...
Il s'agit donc de pouvoir convenablement échantillonner selon la mesure de \nom{Gibbs}, c'est-à-dire d'avoir un algorithme qui tire des configurations $\sigma$ selon leur probabilité $\mu(\sigma)$ de \nom{Gibbs} (\emph{i.e.} un \emph{\nom{Gibbs} sampler} en anglais).
%De nombreux algorithmes de type Monte Carlo \nom{Markov} Chain (MCMC) ont été proposés pour obtenir un \textit{\nom{Gibbs} sampler}.
%On peut grossièrement distinguer deux catégories d'algorithmes MCMC pour le \textit{\nom{Gibbs} sampling}.
%(L'algorithme de \nom{Louvain} précité \cite{Louvain2008} pour maximiser la modularité n'est pas un \textit{\nom{Gibbs} sampler} néanmoins, il cherche aussi à minimiser une énergie de \nom{Gibbs} comme \Eq\ref{Energie}. Son efficacité vient du fait qu'il applique d'abord des changements locaux de type single-spin update, puis une fois un minimum local d'énergie trouvé, il procède à de macro-changements en regroupant ensemble tous les nœuds d'un même cluster.)



\subsection{Le théorème ergodique et les chaînes de \nom{Markov}}

Pour échantillonner de la sorte, l'on recourt souvent à des méthodes de Monte-Carlo par chaînes de \nom{Markov} (\emph{\nom{Markov} Chain Monte Carlo}, ou MCMC en anglais) \cite{Robert1996MCMC,JustinSaintFlour2025}.

Le principe consiste à construire une chaîne de \nom{Markov} $(\Sigma_t)_{t\in\mathbb{N}}$ définie par un noyau de transition $P$, telle que sa distribution stationnaire soit exactement la mesure cible $\mu$. Cette invariance est souvent obtenue en imposant une condition de réversibilité locale (\emph{detailed balance} en anglais).
...
Grâce au théorème ergodique \cite{JustinSaintFlour2025}, si la chaîne simulée sur cet espace fini est irréductible de mesure invariante $\mu$, cette espérance peut être estimée presque sûrement par la moyenne empirique le long de la trajectoire $(\Sigma_t)$ :
\[
...
Ainsi font les dynamiques de \nom{Metropolis--Hastings} et de \nom{Glauber} \cite{GlauberMetropolisDiaconis2009}.


Un algorithme fameux de détection de communautés, dit de \nom{Louvain} \cite{Louvain2008}\footnote{Pour une approche plus physique statistique, voir le \emph{Constant \nom{Potts} Model} \cite{ConstantPottsModel}, intégré à un raffinement de l'algorithme de \nom{Louvain} : l'algorithme de \nom{Leiden} \cite{Leiden2019}.}, consiste à maximiser une certaine \emph{modularité}, qui n'est autre que l'opposé d'une énergie sur les configurations.
L'efficacité de l'algorithme de \nom{Louvain}  vient du fait qu'il applique d'abord une approche de mise à jour locale, puis une fois un minimum local d'énergie trouvé, il procède à de macro-changements en regroupant ensemble tous les nœuds d'un même cluster. 

\subsection{Dynamiques de clusters face à la frustration}

Le théorème ergodique garantit la convergence presque sûre. Le problème en pratique vient de la vitesse à laquelle cette convergence se produit \cite{Robert1996MCMC} (le temps de mélange de la chaîne de \nom{Markov}, ou \emph{mixing time} en anglais). 
...
L'algorithme de \SW{} \cite{SwendsenWang}, généralisé par \nom{Edwards \& Sokal} \cite{EdwardsSokal}, s'appuie sur le modèle de percolation des amas aléatoires (\emph{random-cluster model}) \cite{FortuinKasteleyn, Grimmett2006} pour créer des dynamiques de clusters. %Nous y reviendrons plus en détail dans le chapitre consacré.
...
Pour surmonter cet obstacle, inspirés par les travaux de \nom{Kandel, Ben-Av et Domany} \cite{KBD1992}, nous proposerons d'étendre la dynamique de \SW{} en y intégrant des interactions d'ordre supérieur. 
...
Toutefois, leur utilisation se heurte à des limites pratiques sévères, récemment soulignées par \nom{Zhang} \emph{et al}. \cite{Zhang2025Review} : 
\begin{enumerate}
...
Citons \emph{exempli gratia} le filtre de \nom{Sato} \cite{Sato1998}. Le plus fameux est peut-être le filtre de \nom{Frangi} \cite{Frangi1998} qui analyse de façon multi-échelle les valeurs propres de la matrice hessienne ; le filtre de \nom{Frangi} est un standard pour rehausser (puis détecter) les structures tubulaires. 
...
Le chapitre~\ref{chap:frangi_fissures} propose de généraliser ce filtre en déplaçant la réponse, de pixélique qu'elle était, vers un (hyper-)graphe spatial de similarité entre paires/triplets de pixels. 
...
Une fois cette information bien encodée dans un graphe de similarité, baptisé graphe de \nom{Frangi}, nous avons développé un algorithme -- lointain cousin de \HGP-Clusterer -- pour l'extraction du réseau de fissures. 
...
% C'est le point de départ du chapitre suivant : donner un cadre bayésien à la détection de communautés sur graphes, puis construire des dynamiques de clusters de type \nom{Swendsen--Wang} adaptées à des interactions plus riches que de simples arêtes.
...
% La revue récente de \nom{Zhang} \emph{et al.} insiste précisément sur ce déplacement du domaine : on ne cherche plus seulement de bons scores sur un jeu de données donné, mais des méthodes plus sobres en annotation et plus généralisables \cite{Zhang2025Review}.

% Ce chapitre introductif fixe donc le vocabulaire et les motivations de la partie.
% La première section rappelle brièvement le principe des chaînes de \nom{Markov} et de la méthode \nom{Monte-Carlo} par chaînes de \nom{Markov}.
...
% % Même l'écriture de la constante de normalisation d'une loi de \nom{Gibbs} devient impraticable.
% Il faut donc remplacer le calcul exact par une exploration aléatoire bien contrôlée.




% Une façon classique de procéder consiste à construire une chaîne de \nom{Markov} (c'est-à-dire un processus aléatoire dont les transitions ne dépendent que de l'état actuel) qui se promène dans l'espace des configurations.
...
% C'est le principe des méthodes \nom{Monte-Carlo} par chaînes de \nom{Markov}, ou MCMC \cite{Metropolis1953,Hastings1970,GlauberMetropolisDiaconis2009}.

% \subsection{Chaînes de \nom{Markov} : noyau, invariance, réversibilité}
...
% Une chaîne de \nom{Markov} homogène sur $\mathcal E$ est décrite par un noyau de transition
% \[
%     P(x,y)=\mathbb P(X_{t+1}=y\mid X_t=x),
%     \qquad x,y\in\mathcal E.
% \]
% La propriété de \nom{Markov} signifie que, conditionnellement à l'état présent, le futur ne dépend pas du passé.
...
% Le paradigme MCMC consiste à partir d'une loi cible $\pi$ que l'on ne sait pas échantillonner directement, puis à construire une chaîne de \nom{Markov} ayant $\pi$ pour loi invariante.
% L'exemple fondateur est l'algorithme de \nom{Metropolis}, introduit pour des simulations de physique statistique \cite{Metropolis1953}, puis généralisé par \nom{Hastings} \cite{Hastings1970}.
...
% Dans les modèles de \nom{Gibbs}, la loi cible s'écrit souvent sous la forme
% \begin{equation}
%     \pi_\beta(\sigma)
%     =
...
% La dynamique de \nom{Swendsen--Wang} en est l'exemple classique \cite{SwendsenWang}.
% Elle repose sur la représentation en amas aléatoires de \nom{Fortuin--Kasteleyn} \cite{FortuinKasteleyn,Grimmett2006} et sur le formalisme de \nom{Edwards--Sokal} \cite{EdwardsSokal}.
...
% Cette stratégie peut changer brutalement l'état global du système tout en préservant la loi de \nom{Gibbs}.
...
% Elle fait aussi apparaître un lien profond entre échantillonnage et percolation : les clusters gelés par la dynamique ne sont pas seulement un artifice algorithmique, ils donnent une lecture géométrique des corrélations de la loi de \nom{Gibbs}.
...
% \Contributions{Le rôle de cette première section n'est pas de refaire une théorie générale des chaînes de \nom{Markov}. Il est plus modeste : rappeler le principe des méthodes MCMC, expliquer pourquoi les lois de \nom{Gibbs} apparaissent naturellement dans la détection de communautés sur graphes, et préparer l'introduction des dynamiques de clusters d'ordre supérieur du chapitre suivant.}
...
% Le filtre de \nom{Frangi}, introduit pour rehausser les structures vasculaires en imagerie médicale \cite{Frangi1998}, s'appuie sur les valeurs propres de la hessienne lissée d'une image.
...
% Des travaux voisins, comme ceux de \nom{Sato} \emph{et al.} \cite{Sato1998}, appartiennent à la même famille d'outils multi-échelles.
...
% La revue de \nom{Zhang} \emph{et al.} résume bien l'évolution actuelle du domaine \cite{Zhang2025Review}.
...
% Dans le cas du filtre de \nom{Frangi}, l'hypothèse est que la structure recherchée est localement curviligne et possède un comportement de hessienne compatible avec une ligne sombre ou claire.
...
% Nous partons du filtre de \nom{Frangi}, mais nous le déplaçons du pixel vers le graphe.
...
% Le graphe de \nom{Frangi} produit un squelette éditable ; un expert peut le corriger plus vite qu'il ne dessinerait une vérité terrain depuis une image brute.
% Une autre piste consiste à injecter explicitement des opérateurs différentiels dans les architectures apprenantes.
% Des travaux récents autour de neurones inspirés du filtre de \nom{Frangi} vont dans cette direction \cite{NeuroneFrangi2023}.
...
% La loi cible est une postérieure, souvent de type \nom{Gibbs}, et l'enjeu est de l'échantillonner efficacement.
% Nous y introduisons une famille de dynamiques de clusters d'ordre supérieur, inspirées de \nom{Swendsen--Wang}, et nous montrons comment elles permettent de mieux traiter la frustration dans des graphes spatiaux.
% Ce chapitre reprend et développe les travaux menés avec \nom{Nahuel Soprano-Loto} et \nom{Konstantin Avrachenkov} \cite{BibiMonterey,BibiInformationInference}.

% \paragraph{Chapitre~\ref{chap:frangi_fissures}.}
% Le second chapitre technique applique le langage des graphes à des images.
% Nous partons du filtre de \nom{Frangi} et construisons un graphe de similarité entre pixels afin d'extraire des réseaux de fissures sans entraînement supervisé.
...
% \Synthese{Cette partie étend le fil directeur du manuscrit au-delà des nuages de points euclidiens. Dans les graphes signés, la structure est portée par une loi de \nom{Gibbs} et par des dynamiques d'échantillonnage. Dans les images, la structure est portée par des réponses différentielles locales et par un graphe de compatibilité entre pixels. Dans les deux cas, l'idée reste la même : exploiter explicitement la structure des données plutôt que l'écraser dans une représentation trop pauvre.}
