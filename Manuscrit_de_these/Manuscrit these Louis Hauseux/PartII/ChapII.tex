...
Cette façon de connecter entre eux des simplexes (vu comme des cliques sur sur un graphe) au moyen d'une clique plus grande est appelée \emph{clique percolation} \cite{KCliquePercolation}. Pour le cas plus général des complexes simpliciaux, \nom{Atkin} avait déjà défini en 1972 ce qu'il nommait la $q$-\emph{connectivity} \cite{Atkin1972}. Aujourd'hui, l'usage serait plutôt d'employer le terme \emph{(up) face percolation} \cite{HirschValesin2025}.}
...
\begin{Preuve}[\nom{Preuve du \Th~\ref{th:cech_kpolyedres_knn}}]
