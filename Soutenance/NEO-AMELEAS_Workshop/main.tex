...
\hypersetup{
   ...
   pdftitle    = {From Statistics to (Hyper)graphs: The Benefit of Higher-Order Interactions for a Good Hierarchical Structure},
...
\title[From statistics to (hyper)graphs]{\large From statistics to (hyper)graphs.\\[6pt] \normalsize \normalfont On the benefit of higher-order interactions for a good hierarchical structure}
\subtitle{\small Presentation at the NEO-AMELEAS Workshop}
\date[3 July 2026]{3 July 2026}
\author[L. Hauseux]{%
...
\begin{frame}{From Robust Single-Linkage to (H)DBSCAN}
    ...
    \begin{itemize}
        \setlength{\itemsep}{6pt}
        \item Overly restrictive for peripheral points.
        \item Discards border points without $K$ neighbors, reducing recall by misclassifying them as noise.
    \end{itemize}
    ...
    \begin{itemize}
        \setlength{\itemsep}{6pt}
        \item Distinguishes \textbf{core points} ($r_K(x) \le r$) from \textbf{border points} (reachable from a core but lacking $K$ neighbors).
        \item Improves classification of cluster boundaries.
    \end{itemize}
    
    \vspace{0.4cm}
    \begin{block}{From DBSCAN to HDBSCAN}
        DBSCAN requires a single global threshold $r$. 
        \textbf{HDBSCAN} \cite{HDBSCAN} (Campello \emph{et al.}, 2013) varies this threshold continuously to eliminate $r$ and construct a complete hierarchy.
    \end{block}
\end{frame}
    ...
    \begin{columns}[c, onlytextwidth]
        \begin{column}{0.48\textwidth}
            \begin{itemize}
                ...
                \begin{itemize}
                    \item Comparison of the cluster hierarchy (produced by HGP-Clusterer) with the theoretical 3D model of the ship (\copyright\ \textsc{Naval Group}).
                \end{itemize}
                \item \textbf{Hierarchy Exploration}:
                \begin{itemize}
                    \item A cost function penalizes hybrid nodes containing both reference and anomaly points.
                \end{itemize}
        ...
        \begin{column}{0.48\textwidth}
            \begin{center}
                ...
                \vspace{0.3cm}
                \scriptsize \textbf{Anomaly isolation result} (reference model in red, anomalies in distinct colors).
            \end{center}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Temporal Instance Tracking (SemanticKITTI)}
    \begin{columns}[c, onlytextwidth]
        ...
        \begin{column}{0.48\textwidth}
            \begin{center}
                \href{https://semantic-kitti.org/}{\includegraphics[width=0.95\textwidth]{imgs/kitti_3d_prior_3.png}}
