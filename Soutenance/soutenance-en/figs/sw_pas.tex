% One signed Swendsen--Wang step: configuration -> frozen edges -> recolouring.
% Convention (manuscript, PartIII/ChapII; NIM talk of 25 June 2026):
%   solid line = attractive interaction, dashed line = repulsive interaction,
%   bold line  = satisfied edge, bold red line = frozen edge.
% Configuration: a=+, b=+, c=-, d=-, e=-, f=+.
    \begin{tikzpicture}[x=1cm,y=1cm, scale=0.75, every node/.style={scale=0.75}]
      ...
      \begin{scope}[shift={(0,-0.90)},every node/.style={scale=0.75,font=\scriptsize}]
        ...
        \node[anchor=west] at (3.68,0) {repulsion};
        \draw[posedge,freeze] (5.8,0)--(6.5,0);
        \node[anchor=west] at (6.58,0) {satisfied edge};
        \draw[freeze,inria-rouge] (9.4,0)--(10.1,0);
        \node[anchor=west] at (10.18,0) {frozen edge};
      \end{scope}
    \end{tikzpicture}
