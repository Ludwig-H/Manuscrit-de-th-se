...
\begin{tikzpicture}[scale=0.82, >=latex, font=\small]
    ...
    \node[gris_fonce_inria, right] at (7.1, 2.55) {coupe \textbf{intermédiaire}};

    % Étiquettes des deux extrémités
    \node[right] at (7.1, 0) {les \textbf{feuilles} $\Rightarrow$ \textsc{Glauber}};
    \node[right] at (7.1, 4.0) {les \textbf{racines} $\Rightarrow$ \textsc{Swendsen}--\textsc{Wang}};
\end{tikzpicture}
