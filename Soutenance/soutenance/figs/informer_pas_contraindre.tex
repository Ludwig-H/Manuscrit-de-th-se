...
% (biais additif indexé par la distance dans l'arbre).
                \begin{tikzpicture}[xscale=1.0, yscale=1.0]
                    ...
                    \begin{scope}[shift={(0,0)}]
                        \node[titre, text=gris_fonce_inria] at (1.40,3.05) {\textsc{contraindre} (HSA)};
                    ...
                    \begin{scope}[shift={(6.30,0)}]
                        \node[titre, text=inria-rouge] at (1.40,3.05) {\textsc{informer} (biais additif)};
                        \foreach \i in {0,...,7} {
                            \foreach \j in {0,...,7} {
                        ...
                        \draw[cadre] (0,0) rectangle (2.80,2.80);
                        \node[sous] at (1.40,-0.22)
                            {chaque coefficient reste \textbf{libre} ;\\on ajoute un terme appris\\indexé par $d_T(i,j)$};
                    \end{scope}
                    ...
                    \node[font=\scriptsize, text=inria-rouge, anchor=north, align=center] at (7.70,-1.42)
                        {$q_i^{\top}k_j/\sqrt d \;+\; b\bigl(d_T(i,j)\bigr)$\\[3pt]
                         $\Rightarrow$ \textbf{renseigne} l'attention\\{\tiny $\sim\!200$ paramètres, initialisés à zéro}};
